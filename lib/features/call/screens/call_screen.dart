// -----------------------------
// CALL SCREEN (rewritten + commented)
// -----------------------------
// Note: variable names and public API are unchanged as requested.
...
/// ------------------------------------------------------------
///  UNIVERSAL CALL SCREEN (Video + Audio) - REWRITTEN
/// ------------------------------------------------------------
/// - Preserves your public API / variable names.
/// - Fixes UID collision by assigning unique UIDs for caller vs receiver.
/// - Keeps token fetching/renewal logic intact but organized.
/// - Detailed comments to explain every important step.
/// ------------------------------------------------------------
...
class _CallScreenState extends ConsumerState<CallScreen> {
  ...
  /// Remote user uid assigned by Agora when remote joins
  int? _remoteUid;

  /// Local join state (true when onJoinChannelSuccess fired)
  ...
  /// Local UID used when joining Agora channel
  /// NOTE: we intentionally keep the field name `localUid` unchanged.
  int localUid = 0;

  /// Local backend server (token generator). Use 10.0.2.2 for emulator.
  ...
  void initState() {
    ...
    // ===== IMPORTANT FIX =====
    // Ensure caller and callee generate different UIDs.
    // We keep your naming scheme and simply pick the appropriate id
    // depending on who this device represents.
    final bool isCaller = widget.call.hasDialled;

    // Keep your modulo approach for limiting uid size, but use different
    // source for caller vs callee so they won't collide.
    localUid = (isCaller
            ? widget.call.callerId.hashCode
            : widget.call.receiverId.hashCode) %
        1000000;

    developer.log('Local UID set to: \$localUid (isCaller: \$isCaller)');

    _initAgora();
  }

  /// Initialize Agora engine, request permissions and join channel.
  Future<void> _initAgora() async {
    // Request microphone (and camera if video) permissions first.
    await [
      Permission.microphone,
      if (widget.isVideoCall) Permission.camera,
    ].request();

    // Create the engine and initialize with your App ID.
    _engine = createAgoraRtcEngine();
    await _engine.initialize(RtcEngineContext(appId: AgoraConfig.appId));

    // Register event handlers to track channel events.
    _engine.registerEventHandler(
      RtcEngineEventHandler(
        onJoinChannelSuccess: (RtcConnection connection, int elapsed) {
          // local user successfully joined the channel
          setState(() => _localUserJoined = true);
          developer.log('✅ Joined channel: \${widget.channelId} as UID \$localUid');
        },
        onUserJoined: (RtcConnection connection, int remoteUid, int elapsed) {
          // remote participant joined — store their uid so we can show remote view
          setState(() => _remoteUid = remoteUid);
          developer.log('👤 Remote user joined: \$remoteUid');
        },
        onUserOffline: (RtcConnection connection, int remoteUid, UserOfflineReasonType reason) {
          // remote user left the call (or went offline)
          setState(() => _remoteUid = null);
          developer.log('🚪 Remote user left: \$remoteUid');
        },
        onTokenPrivilegeWillExpire: (RtcConnection connection, String token) {
          // SDK informs us token is about to expire — refresh it.
          developer.log('⚠️ Token about to expire — refreshing...');
          _refreshAgoraToken();
        },
      ),
    );
    ...
    // Finally, attempt to join the channel (with retry logic handled inside)
    await _joinChannelWithLogging();
  }

  /// Fetch RTC token and join channel. If join fails due to token issues,
  /// the method tries to refresh the token and retry once.
  Future<void> _joinChannelWithLogging() async {
    ...
    if (_rtcToken == null) {
      developer.log('❌ Could not fetch token — aborting join');
      return;
    }

    try {
      developer.log('🔌 Attempting to join channel with uid: \$localUid');
      ...
      developer.log('✅ Join request submitted');

      // Start scheduled refresh (keeps renewing tokens periodically)
      _startTokenAutoRefresh();
    } catch (e, stack) {
      developer.log('❌ Join failed: \$e', error: e, stackTrace: stack);

      // Attempt one refresh and retry — helpful for race conditions with tokens.
      developer.log('🔄 Trying to refresh token and retry join');
      await _refreshAgoraToken();
      await Future.delayed(const Duration(seconds: 2));

      // Try to join once more (if token updated)
      try {
        await _engine.joinChannel(
          token: _rtcToken!,
          channelId: widget.channelId,
        ...
        developer.log('❌ Retry join also failed: \$e2', error: e2, stackTrace: stack2);
      }
    }
  }
  ...
  Future<String?> _fetchAgoraToken(String channelId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        developer.log('✅ Token received');
        return data['token'];
      } else {
        developer.log('❌ Token request failed (code: \${response.statusCode})');
      }
    } catch (e, stack) {
  ...
  /// Refresh token by calling your backend, then notify the Agora engine.
  Future<void> _refreshAgoraToken() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Give the new token to Agora to continue the session.
  ...
  /// Start automatic token refresh every 50 minutes (typical token lifetime pattern)
  ...
  void dispose() {
    _tokenRefreshTimer?.cancel();
    // Safely leave and release the engine
    try {
      _engine.leaveChannel();
      _engine.release();
    } catch (e) {
      developer.log('⚠️ Error on dispose: \$e');
    }
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SafeArea(
        child: Stack(
          children: [
            if (widget.isVideoCall) _buildVideoLayout() else _buildAudioLayout(),

            // End Call Button — leaves channel and notifies call controller
            Align(
  ...
  Widget _buildVideoLayout() {
    // show loader until local user has joined
    if (!_localUserJoined) return const Loader();

    return Stack(
      children: [
        // Remote user's video (if available)
        _remoteUid != null
            ? AgoraVideoView(
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              // show Connected when remote uid has been detected
              _remoteUid != null
                  ? 'Connected'
                  : _localUserJoined
                      ? 'Ringing...'
...
class _CallScreenState extends ConsumerState<CallScreen> {
  ...
  /// change it with with localhost ip address when using real devices
  final String baseUrl = 'http://10.0.2.2:3000';
...
void initState() {
  ...
  // Caller should use callerId
  // Receiver should use receiverId
  final isCaller = widget.call.hasDialled;

  localUid = (isCaller
      ? widget.call.callerId.hashCode
      : widget.call.receiverId.hashCode) % 1000000;// simple hash for uniqueness
      ...
      child: Center(
        child: Column(
          ...
          children: [
            const CircleAvatar(
              radius: 60,
             // backgroundImage: AssetImage('assets/images/default_avatar'),
             backgroundImage: NetworkImage(
                            'https://png.pngitem.com/pimgs/s/649-6490124_katie-notopoulos-katienotopoulos-i-write-about-tech-round.png',
                          ),
            ),
